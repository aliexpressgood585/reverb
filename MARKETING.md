...
1. `04-walking` — the wavefront line sweeping the platform tile
2. `08-enemy` — a creature caught mid-stride in rust orange, rings at its feet
3. `07-gunshot` — the whole room, white, for a quarter of a second
4. `09-tunnel` — ballast lit up, the bore running away into black
5. `05-imprint-only` — the room as memory, almost gone
6. `01-title` — MAKE NO SOUND
